...
| CS-12 | In progress | Sonnet implementer + Luna fallback implementer/reviewers + Codex | Both funding rails, recovery, and reconciliation | CS-10, CS-11 | Task 4 accepted: durable Cycles Ledger withdrawal, protected self-recovery, exact retry/quarantine, source reservation, and evidence-required reconciliation core. Initial Sonnet implementation was corrected after adversarial review; final dual Luna closeout passed with 542 tests, scoped fmt, host check, release Wasm, and diff validation. ICP/CMC remains Task 5. |
