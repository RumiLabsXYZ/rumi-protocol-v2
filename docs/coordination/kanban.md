...
| CS-54 | Review | Codex + Luna fallback reviewers | Interoperate with wallet ICRC-21 consent preflight without auto-opening operator auth | CS-50 | Exact ICRC-21 `FieldsDisplay` and error surface added; operator session is bound to wallet type plus principal and invalidated on every observed connection transition. 598 Rust tests, strict Clippy, 9 focused UI tests, and frontend production build passed. Fresh reviews and live upgrade/readback pending. |
