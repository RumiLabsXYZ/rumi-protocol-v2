...
- `rumi_protocol_backend.get_all_vaults()` returns `vec CandidVault` (fields: owner, vault_id, collateral_amount, borrowed_icusd_amount, accrued_interest, collateral_type)
- `rumi_protocol_backend.get_collateral_totals()` returns `vec CollateralTotals` (fields: collateral_type, price, vault_count, total_collateral, total_debt, symbol, decimals)
...
| `three_pool_lp_supply_e8s` | `Option<u128>` | `three_pool.get_pool_status().lp_total_supply` |

**3pool `nat` to `u128` conversion**: The 3pool `.did` types `balances`, `virtual_price`, and `lp_total_supply` are Candid `nat` (arbitrary precision). The source wrapper performs a checked conversion to `u128`, returning `Err` on overflow. With 8-decimal tokens, overflow is not practically possible, but the conversion is explicit.

**Note**: `DailyStabilityRow.total_deposits_e8s` intentionally duplicates `DailyTvlRow.stability_pool_deposits_e8s`. They serve different query endpoints and having the data in both logs avoids cross-log joins.

The collector makes three concurrent async calls (backend, stability pool, 3pool) via `futures::join!`. If stability pool or 3pool calls fail, the row is still written with the backend data populated and the failed fields as `None`. Errors increment the per-source error counter. This is a change from Phase 1 where the whole row was skipped on error. The rationale: partial data is better than no data for a composite snapshot.
...
**Data sources**: Two concurrent calls:
- `backend.get_all_vaults()` for individual vault data (collateral_amount, borrowed_icusd_amount, accrued_interest, collateral_type)
- `backend.get_collateral_totals()` for per-collateral USD prices (the `price: float64` field)

`CandidVault` does not include a precomputed CR. The collector computes each vault's CR as: `(collateral_amount * price) / (borrowed_icusd_amount + accrued_interest)`, expressed in basis points. Prices come from `CollateralTotals.price` (one per collateral type), which is the backend's most recent XRC price.
...
| `price_usd_e8s` | `u64` | Collateral price in USD (e8s) at snapshot time |

The protocol-wide median is the median across all individual vault CRs, not a median of per-collateral medians.

**Error handling**: If `get_all_vaults` or `get_collateral_totals` fails, no row is written (both calls are required). Error counter incremented, error logged.
...
The collector calls `stability_pool.get_pool_status()` and maps the response directly.

**Error handling**: If the call fails, no row is written. Error counter incremented, error logged.

## Source wrappers

Three new files in `sources/`:
...
**`sources/three_pool.rs`**: Wrapper for `get_pool_status()`. Returns a `ThreePoolStatusSubset` with balances (as `Vec<u128>`), virtual_price (as `u128`), lp_total_supply (as `u128`). Performs checked `Nat -> u128` conversion on each field.

**`sources/backend.rs`** (extend existing): Add wrapper for `get_all_vaults()` and `get_collateral_totals()`.

## Storage

**MemoryIds** (already reserved in `storage.rs`):
- `DAILY_VAULT_SNAPSHOTS`: MemoryIds 12 (idx) and 13 (data) (`MEM_DAILY_VAULTS_IDX` / `MEM_DAILY_VAULTS_DATA`)
- `DAILY_STABILITY`: MemoryIds 24 (idx) and 25 (data) (`MEM_DAILY_STABILITY_IDX` / `MEM_DAILY_STABILITY_DATA`)

Each StableLog requires a paired index/data MemoryId, matching the existing `DAILY_TVL_LOG` pattern (MemoryIds 10/11). The MemoryId constants already exist in `storage.rs`; we just need to instantiate the StableLogs.

No new MemoryIds needed for the TVL extension since the row struct just gets wider.
...
All three collectors fire on the existing daily (86400s) timer in `timers.rs`. The `daily_snapshot` function runs them concurrently via `futures::join!` since they are independent. Each collector's failure is isolated (logged + error counter incremented) and does not block the others.
...
2. **Vault snapshot**: Deploy analytics + mock backend with fixture vaults (multiple collateral types), advance past daily tick, query `get_vault_series`, verify per-collateral stats and protocol-wide aggregates (CR, min/max/median).
