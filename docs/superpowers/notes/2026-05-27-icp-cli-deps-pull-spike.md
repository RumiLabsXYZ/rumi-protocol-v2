...
1. **Accept fresh local IDs.** PocketIC integration tests already use this pattern. Rumi's backend builds the XRC/II principals from config (set at install time), not from hardcoded IDs. Update the integration-test wiring to read `.icp/cache/mappings/<env>.ids.json` (icp-cli's analogue of `.dfx/local/canister_ids.json`). Note the shape difference: icp-cli writes a flat `{ "name": "principal" }` map per environment, whereas dfx wrote env-nested `{ "name": { "local": "principal" } }`.
...
- **Canister ID reassignment.** Rumi's PocketIC integration tests already accept whatever IDs are allocated at install. The `vault_frontend` build embeds canister IDs into the JS bundle via a `declarations/` generation step. icp-cli writes its allocated IDs to `.icp/cache/mappings/<env>.ids.json` as a flat `{ "name": "principal" }` map. We will need to wire the frontend build to read from `.icp/cache/mappings/local.ids.json` instead of `.dfx/local/canister_ids.json` (and adjust for the flatter JSON shape). This is a Phase 1 task, not a blocker for Phase 0.
