...
**`disable_chain` is the reversible emergency risk/XRC stop.** It blocks new opens and additional borrows, and it unmanages the native pair (the timer stops fetching it and manual pricing reopens). It deliberately does NOT touch exit paths: withdraw, close and repay keep working on a Disabled chain, and it does not stop the observer/settlement workers or liquidation processing for already-open vaults.

**Manual rebaseline is available only while Disabled**, and `enable_chain` resumes automatic XRC authority. The full recovery loop, which is also what the PocketIC fixtures exercise:
```
