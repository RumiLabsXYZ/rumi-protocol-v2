...
#[test]
fn backstop_retries_when_supply_state_changes_during_probe() {
    use super::deposit_watch::backstop_probe_requires_cursor_retention;

    assert!(!backstop_probe_requires_cursor_retention(1_000, 1_000, false));
    assert!(backstop_probe_requires_cursor_retention(1_000, 1_100, false));
    assert!(backstop_probe_requires_cursor_retention(1_000, 900, false));
    assert!(backstop_probe_requires_cursor_retention(1_000, 1_000, true));
}
