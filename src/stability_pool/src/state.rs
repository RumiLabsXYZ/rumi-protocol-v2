...
impl StabilityPoolState {
    ...
    /// Every user's pending native-SOL payouts, ordered by claim id (claim ids
    /// are allocated monotonically, so this is also oldest-first). Drives the
    /// auto-settlement sweep (mirrors `all_native_xrp_pending_payouts`).
    pub fn all_native_sol_pending_payouts(&self) -> Vec<(Principal, NativeSolPendingPayout)> {
        let mut all: Vec<(Principal, NativeSolPendingPayout)> = self
            .deposits
            .iter()
            .flat_map(|(user, pos)| {
                pos.pending_native_sol_payouts
                    .iter()
                    .flat_map(|payouts| payouts.values())
                    .map(|p| (*user, p.clone()))
            })
