  ...
  async function loadEvent() {
    ...
    try {
      if (parsed.source === 'backend') {
        const [results, vaults, count] = await Promise.all([
          publicActor.get_events({ start: BigInt(parsed.id), length: 1n, types: [], principal: [], collateral_token: [], time_range: [], min_size_e8s: [], admin_labels: [] }),
    ...
    try {
      ...
      if (parsed.source === 'backend') {
        ...
        const results = await publicActor.get_events({ start, length: len, types: [], principal: [], collateral_token: [], time_range: [], min_size_e8s: [], admin_labels: [] });
