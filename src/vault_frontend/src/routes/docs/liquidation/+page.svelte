<script lang="ts">
  ...
  $: penaltyPct = ((liquidationBonus - 1) * 100).toFixed(0);
  $: penaltyMult = (liquidationBonus * 100).toFixed(0);
...
<article class="doc-page">
  ...
  <section class="doc-section">
    ...
    <p>If the requested amount is less than this cap, the liquidator pays their chosen amount. The liquidator receives collateral proportional to the debt they repay, plus the same {penaltyPct}% penalty. Partial liquidations leave the vault open with reduced debt and reduced collateral.</p>
  </section>
  ...
  <section class="doc-section">
    ...
    <p>A liquidator repays your 70 icUSD debt and receives {exSymbol} worth ${(70 * liquidationBonus).toFixed(0)} (70 &times; {liquidationBonus.toFixed(2)}, including the {penaltyPct}% penalty). That's {(70 * liquidationBonus / 7).toFixed(1)} {exSymbol} at $7/{exSymbol} — but you only have 10 {exSymbol}, so the liquidator gets all 10 {exSymbol}. Your vault is closed. You keep the 70 icUSD you originally borrowed, but your {exSymbol} is gone.</p>
  </section>
