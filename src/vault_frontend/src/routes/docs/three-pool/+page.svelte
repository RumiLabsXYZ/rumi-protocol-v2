...
<article class="doc-page">
  ...
  <section class="doc-section">
    ...
    <p>To swap one stablecoin for another, you approve the input token for spending (ICRC-2) and then call the pool's <code>swap</code> function specifying the input token, output token, amount, and minimum output. The pool calculates the output using the StableSwap invariant and transfers the result to your wallet. Outbound transfers are net of the output token's ledger fee, so the amount that lands in your wallet is the quoted output minus that network fee; your minimum-output bound is checked against the net amount.</p>
  ...
  <section class="doc-section">
    ...
    <p>When you add liquidity, you receive 3USD, a token minted by the 3pool canister with 8 decimals. 3USD represents your proportional share of the pool's total value. It is a fully standard token implementing <strong>ICRC-1, ICRC-2, and ICRC-3</strong> (transfers, approvals, and a verifiable block log with its own index canister), so you can hold it in any compatible wallet, transfer it, approve it for spending, or deposit it in the <a href="/docs/stability-pool" class="doc-link">stability pool</a>. Because its value grows with the pool's virtual price, 3USD works as a yield-bearing stable asset, and the <a href="/3usd" class="doc-link">3USD</a> page offers one-click mint and redeem against any of the pool's stablecoins.</p>
  ...
  <section class="doc-section">
    ...
    <p>The remaining interest is split between the <a href="/docs/stability-pool" class="doc-link">stability pool</a> (<span class="live">{splitPct('stability_pool')}</span>), <a href="/docs/swap" class="doc-link">AMM</a> liquidity providers (<span class="live">{splitPct('amm1')}</span>), and the protocol treasury (<span class="live">{splitPct('treasury')}</span>). This split is admin-configurable; see <a href="/docs/parameters" class="doc-link">Protocol Parameters</a> for current values.</p>
  </section>
  ...
  <section class="doc-section">
    ...
    <p><strong>Smart contract risk:</strong> The 3pool is a separate canister from the main Rumi Protocol. It has its own code and its own risks. A bug in the pool's StableSwap math, rounding, or token transfer logic could result in loss of funds. The pool is covered by the protocol's recurring security reviews (reports published at <a href="https://rumiprotocol.com/security" class="doc-link" target="_blank" rel="noopener">rumiprotocol.com/security</a>), but it has not been audited by a traditional human-led security firm.</p>
