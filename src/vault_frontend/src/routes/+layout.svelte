<script lang="ts">
  ...
  import SolPendingDepositBanner from "../lib/components/vault/SolPendingDepositBanner.svelte";
  import "../app.css";
...
<style>
  ...
  .top-wordmark { font-family:'Circular Std','Inter',sans-serif;font-size:1.0625rem;font-weight:500;letter-spacing:0.12em;color:var(--rumi-text-primary); }
  ...
  /* ── Main content ── */
  .main-content { padding:calc(4.75rem + var(--rumi-xrp-recovery-height, 0px) + var(--rumi-sol-recovery-height, 0px) + var(--rumi-strip-height, 0px)) 2rem 2rem;min-height:100vh;position:relative;z-index:1;max-width:1200px;margin:0 auto; }
  ...
  .footer-link { font-size:0.75rem;color:var(--rumi-text-muted);text-decoration:none;transition:color 0.15s ease; }
  ...
  @media (max-width:768px) {
    ...
    .main-content{padding:calc(4.25rem + var(--rumi-xrp-recovery-height, 0px) + var(--rumi-sol-recovery-height, 0px) + var(--rumi-strip-height, 0px)) 1rem 5rem}
