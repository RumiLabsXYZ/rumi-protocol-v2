  ...
  async function loadAllData() {
    try {
      ...
      if (protocolStatus && status) {
        // Compute pool TVL: sum of all stablecoin balances, normalized to e8s
        let poolTvlE8s = 0;
        for (let i = 0; i < status.balances.length; i++) {
          const token = POOL_TOKENS[i];
          if (token) {
            // Normalize to 8-decimal base (icUSD native)
            const normalized = token.decimals === 8
              ? Number(status.balances[i])
              : Number(status.balances[i]) * 100; // 6-dec → 8-dec
        ...
        theoreticalApy = calculateTheoreticalApy(
          ...
          poolTvlE8s / 1e8, // convert to human-readable units to match totalDebtE8s
        );
      }

      // Prefer theoretical, fall back to VP-based
...
<svelte:head>
  <title>Swap | Rumi Protocol</title>
</svelte:head>

<div class="page-container">
  <div class="page-header">
    <h1 class="page-title">StableSwap</h1>
      ...
      >
        ...
        {#if showApyTooltip}
          <div class="apy-tooltip">
            ...
            <p><strong>LP APY</strong> is earned by liquidity providers from <strong>borrowing interest</strong> donations plus <strong>swap fees</strong>, based on current protocol borrowing rates and pool TVL.</p>
          </div>
        {/if}
      </div>
    {/if}
