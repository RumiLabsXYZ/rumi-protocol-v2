<script>
  import { onMount } from 'svelte';
  import '../app.css';

  let currentPath = '/';
  let appUrl = 'https://app.rumiprotocol.com';
  let mobileMenuOpen = false;

  onMount(() => {
    currentPath = window.location.pathname;
    const match = window.location.hostname.match(/rumiprotocol\.(\w+)$/);
    appUrl = match ? `https://app.rumiprotocol.${match[1]}` : 'https://app.rumiprotocol.com';
    window.addEventListener('popstate', () => {
      currentPath = window.location.pathname;
    });
  });

  function closeMobileMenu() {
    mobileMenuOpen = false;
  }
</script>

<div class="min-h-screen flex flex-col relative z-10">
  <header class="w-full px-6 py-4 sticky top-0 z-50 border-b"
          style="background: rgba(8,11,22,0.85); backdrop-filter: blur(12px); border-color: var(--rumi-border);">
    <div class="max-w-6xl mx-auto flex justify-between items-center">
      <a href="/" class="flex items-center gap-3 no-underline">
        <img src="/rumilogo-vector-v2_inset2.png" alt="Rumi" class="w-9 h-9"/>
        <span class="brand-wordmark">RUMI</span>
      </a>

      <nav class="hidden md:flex items-center gap-8">
        <a href="/" class="nav-link" class:active={currentPath === '/'}>Home</a>
        <a href="/about" class="nav-link" class:active={currentPath === '/about'}>About</a>
        <a href="/Rumi-Protocol-v2-Whitepaper.pdf" target="_blank" class="nav-link">Whitepaper</a>
        <a href="/AVAI_Security_Audit_Rumi_Protocol_Professional.pdf" target="_blank" class="nav-link">Audit</a>
        <a href={appUrl} target="_blank" rel="noopener" class="launch-cta">Launch App</a>
      </nav>

      <div class="flex items-center gap-5">
        <a href="mailto:info@rumiprotocol.com" class="social-icon header-social" aria-label="Email">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
            <polyline points="22,6 12,13 2,6"/>
          </svg>
        </a>
        <a href="https://x.com/rumilabsxyz" target="_blank" rel="noopener" class="social-icon header-social" aria-label="Twitter">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
          </svg>
        </a>
        <a href="https://github.com/RumiLabsXYZ/rumi-protocol-v2" target="_blank" rel="noopener" class="social-icon header-social" aria-label="GitHub">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/>
          </svg>
        </a>
        <a href="https://t.me/+nVydvqMbUWY3MGE5" target="_blank" rel="noopener" class="social-icon header-social" aria-label="Telegram">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
            <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
          </svg>
        </a>
        <a href="https://discord.gg/Ywtu9wUP" target="_blank" rel="noopener" class="social-icon header-social" aria-label="Discord">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
            <path d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189z"/>
          </svg>
        </a>

        <!-- Mobile hamburger -->
        <button class="hamburger" aria-label="Menu" on:click={() => mobileMenuOpen = !mobileMenuOpen}>
          {#if mobileMenuOpen}
            <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
              <path d="M6 6l12 12M6 18L18 6"/>
            </svg>
          {:else}
            <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
              <path d="M4 6h16M4 12h16M4 18h16"/>
            </svg>
          {/if}
        </button>
      </div>
    </div>

    <!-- Mobile nav drawer -->
    {#if mobileMenuOpen}
      <nav class="mobile-nav md:hidden">
        <a href="/" class="mobile-nav-link" class:active={currentPath === '/'} on:click={closeMobileMenu}>Home</a>
        <a href="/about" class="mobile-nav-link" class:active={currentPath === '/about'} on:click={closeMobileMenu}>About</a>
        <a href="/Rumi-Protocol-v2-Whitepaper.pdf" target="_blank" class="mobile-nav-link" on:click={closeMobileMenu}>Whitepaper</a>
        <a href="/AVAI_Security_Audit_Rumi_Protocol_Professional.pdf" target="_blank" class="mobile-nav-link" on:click={closeMobileMenu}>Audit</a>
        <div class="mobile-nav-divider"></div>
        <div class="flex items-center gap-5 px-1 py-1">
          <a href="mailto:info@rumiprotocol.com" class="social-icon" aria-label="Email">
            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
              <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
              <polyline points="22,6 12,13 2,6"/>
            </svg>
          </a>
          <a href="https://x.com/rumilabsxyz" target="_blank" rel="noopener" class="social-icon" aria-label="Twitter">
            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
            </svg>
          </a>
          <a href="https://github.com/RumiLabsXYZ/rumi-protocol-v2" target="_blank" rel="noopener" class="social-icon" aria-label="GitHub">
            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/>
            </svg>
          </a>
        </div>
        <a href="https://app.rumiprotocol.com" target="_blank" rel="noopener" class="mobile-launch-cta" on:click={closeMobileMenu}>Launch App</a>
      </nav>
    {/if}
  </header>

  <main class="flex-grow">
    <slot />
  </main>

  <footer class="w-full px-6 py-8 border-t mt-auto"
          style="background: var(--rumi-bg-surface1); border-color: var(--rumi-border);">
    <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
      <div class="footer-links text-sm" style="color: var(--rumi-text-muted);">
        <span>&copy; 2025-2026 Rumi Labs LLC</span>
        <a href="https://github.com/RumiLabsXYZ/rumi-protocol-v2" target="_blank" rel="noopener" class="footer-link">GitHub</a>
        <a href="/Rumi-Protocol-v2-Whitepaper.pdf" target="_blank" class="footer-link">Whitepaper</a>
        <a href="/AVAI_Security_Audit_Rumi_Protocol_Professional.pdf" target="_blank" class="footer-link">Audit</a>
        <a href="/branding" class="footer-link">Branding</a>
        <a href="mailto:info@rumiprotocol.com" class="footer-link">Contact</a>
      </div>
      <a href={appUrl} target="_blank" rel="noopener"
         class="text-sm font-medium transition-colors" style="color: var(--rumi-action);">
        Launch App →
      </a>
    </div>
  </footer>
</div>

<style>
  .brand-wordmark {
    font-family: 'Circular Std', 'Inter', sans-serif;
    font-size: 1.25rem;
    font-weight: 500;
    color: #e8e4f0;
    letter-spacing: 0.12em;
  }

  .nav-link {
    font-size: 0.875rem;
    font-weight: 500;
    color: var(--rumi-text-secondary);
    text-decoration: none;
    transition: color 0.15s ease;
  }

  .nav-link:hover, .nav-link.active {
    color: var(--rumi-text-primary);
  }

  .launch-cta {
    font-family: 'Circular Std', 'Inter', sans-serif;
    font-size: 0.875rem;
    font-weight: 500;
    padding: 0.5rem 1.25rem;
    background: var(--rumi-action);
    color: var(--rumi-bg-primary);
    border-radius: 0.5rem;
    text-decoration: none;
    transition: all 0.2s ease;
  }

  .launch-cta:hover {
    background: var(--rumi-action-bright);
    box-shadow: 0 0 20px rgba(52,211,153,0.15);
  }

  .footer-link {
    color: var(--rumi-text-muted);
    text-decoration: none;
    transition: color 0.15s ease;
  }

  .footer-link:hover {
    color: var(--rumi-text-secondary);
  }

  .social-icon {
    display: flex;
    align-items: center;
    color: var(--rumi-text-muted);
    transition: color 0.15s ease;
  }

  .social-icon:hover {
    color: var(--rumi-text-primary);
  }

  /* Hide header social icons on mobile, show only in hamburger menu */
  .header-social {
    display: none;
  }
  @media (min-width: 768px) {
    .header-social { display: flex; }
  }

  /* Hamburger button */
  .hamburger {
    display: flex;
    align-items: center;
    justify-content: center;
    background: none;
    border: none;
    color: var(--rumi-text-secondary);
    cursor: pointer;
    padding: 0.25rem;
    transition: color 0.15s ease;
  }

  @media (min-width: 768px) {
    .hamburger { display: none; }
  }

  .hamburger:hover {
    color: var(--rumi-text-primary);
  }

  /* Mobile nav */
  .mobile-nav {
    display: flex;
    flex-direction: column;
    gap: 0.25rem;
    padding: 1rem 1.5rem 1.5rem;
    border-top: 1px solid var(--rumi-border);
  }

  .mobile-nav-link {
    font-size: 0.9375rem;
    font-weight: 500;
    color: var(--rumi-text-secondary);
    text-decoration: none;
    padding: 0.625rem 0;
    transition: color 0.15s ease;
  }

  .mobile-nav-link:hover, .mobile-nav-link.active {
    color: var(--rumi-text-primary);
  }

  .mobile-nav-divider {
    height: 1px;
    background: var(--rumi-border);
    margin: 0.5rem 0;
  }

  .mobile-launch-cta {
    display: block;
    text-align: center;
    font-family: 'Circular Std', 'Inter', sans-serif;
    font-size: 0.9375rem;
    font-weight: 500;
    padding: 0.75rem 1.25rem;
    background: var(--rumi-action);
    color: var(--rumi-bg-primary);
    border-radius: 0.5rem;
    text-decoration: none;
    margin-top: 0.5rem;
    transition: all 0.2s ease;
  }

  .mobile-launch-cta:hover {
    background: var(--rumi-action-bright);
  }

  /* Footer links - responsive wrap */
  .footer-links {
    display: flex;
    flex-wrap: wrap;
    align-items: center;
    justify-content: center;
    gap: 0.5rem 1.5rem;
  }

  @media (min-width: 768px) {
    .footer-links {
      justify-content: flex-start;
    }
  }
</style>
